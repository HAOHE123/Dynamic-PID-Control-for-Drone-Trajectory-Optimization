% MATLAB script to simulate drone trajectories using PID controller without Simulink.
...
for i = 1:length(methods)
    ...
    for k = 1:num_steps - 1
        ...
        acceleration = Kp * error + Ki * error_integral + Kd * error_derivative;
...
xlabel('X-axis');
ylabel('Y-axis');
title('Drone Trajectories with PID Controller');
set(gca, 'FontSize', 12);
hold off;

% Save the figure
exportgraphics(gcf, 'DroneTrajectories_PID.png', 'Resolution', 300);
...
% Save the figure
exportgraphics(gcf, 'SpeedOverTime_PID.png', 'Resolution', 300);
